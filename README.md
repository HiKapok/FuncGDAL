...
FuncGDAL提供了一个框架，可以将你的算法同GDAL读取图像的接口统一起来，同时该框架提供内置的多种分块存取策略、支持就地算法、简化输出图像创建，从而使可以使得你的算法可以写的更简单。该框架目前存在一个问题，无法完美的与类接口结合起来，即无法用成员函数作为算法处理的仿函数，但是static函数是可以的，也可以使用友元函数的办法来补救。
